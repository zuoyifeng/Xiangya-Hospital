$(function () {
                    ...
                    success: function (res) {
                        if (res.status) {
                            ...
                            for (var i = 0, len = res.data.length; i < len; i++) {
                                html += '<option value="' + res.data[i].id + '">' + res.data[i].groupName + '</option>';
                            }
                            $('#usergroup').html(html);
                        } else {
                            myTooltips({
                    ...
                    success: function (res) {
                        if (res.status) {
                            ...
                            for (var i = 0, len = res.data.groups.length; i < len; i++) {
                                if (res.data.groups[i].owner) {
                                    html += '<option selected value="' + res.data.groups[i].id + '">' + res.data.groups[i].name + '</option>';
                                } else {
                                    html += '<option value="' + res.data.groups[i].id + '">' + res.data.groups[i].name + '</option>';
                                }
                            }
                            $('#usergroup').html(html);
                        } else {
    ...
    function mypagination(count, size, current) {
        $.jqPaginator('#pagePaging', {
            totalCounts: parseInt(count) || 1,//数据总数
